...
export const searchAiRecommendations = async (query: string): Promise<GrowthItem[]> => {
  ...
  try {
    ...
    return rawData.map((item: any, index: number) => ({
      ...
      rating: item.rating,
      // Use a generic gradient or placeholder since we can't screenshot search results easily
...
export const generateBlogPost = async (topic: string): Promise<BlogPost | null> => {
    ...
    const systemInstruction = `
        Je bent een professionele SEO redacteur voor Writgo Academy.
        Schrijf een boeiend, informatief artikel over het onderwerp.
        De toon is professioneel maar toegankelijk.
        
        BELANGRIJK OVER DE OPMAAK (HTML):
        De 'content' moet valide HTML zijn, maar ZONDER <html>, <head> of <body> tags.
        Gebruik: <h2>, <h3>, <p>, <ul>, <li>, <strong>, <blockquote>.
    `;

    // Manually define schema to avoid dependency issues
    const responseSchema = {
        type: Type.OBJECT,
        properties: {
            title: { type: Type.STRING, description: "Een SEO-vriendelijke titel" },
            excerpt: { type: Type.STRING, description: "Pakkende inleiding (meta description style)" },
            content: { type: Type.STRING, description: "De volledige artikel inhoud in HTML" },
            category: { type: Type.STRING },
    ...
    try {
        const response = await ai.models.generateContent({
            model: "gemini-2.5-flash",
            contents: `Schrijf een blogartikel over: "${topic}"`,
            config: {
                systemInstruction,
                responseMimeType: "application/json",
                responseSchema
        ...
        return {
            ...
            author: data.author,
            imageUrl: `https://picsum.photos/800/400?random=${Date.now()}`, // Random blog image remains fine
...
export const generateToolMetadata = async (url: string, rawText: string): Promise<GrowthItem | null> => {
    ...
    try {
        ...
        // Enhance author avatar if missing
        ...
        // Try to find OG:Image in rawText first
        ...
        return {
            ...
            reviewConclusion: data.reviewContent?.verdict, // Fallback
